...
{
    let data_reader: list_triple::Reader = try!(ip.read_schema());
...
agent! {
    example_wrangle_stats, edges(list_triple, quadruple)
    ...
    fn run(&mut self) -> Result<()> {
        ...
        {
            let mut quad = raw_ip.build_schema::<quadruple::Builder>();
            quad.set_first(min);
            quad.set_second(max);
        ...
        {
            let mut quad = anonymous_ip.build_schema::<quadruple::Builder>();
            quad.set_first(min);
            quad.set_second(max);
            quad.set_third(average);
            quad.set_fourth(median);
